...
def check_auth_logic_separation(microservices: dict) -> dict:
    ...
    for service_id, service in microservices.items():
        ...
        if not is_centralized:
            has_local_java_impl = any(fi.search_keywords(kw, directory_path, file_extension=["*.java", "*.kt", "*.scala"]) for kw in SECURITY_ANALYSIS_KEYWORDS["decentralized_implementation"]["java_code"])

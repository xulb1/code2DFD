...
def detect_zuul(microservices: dict, information_flows: dict, external_components: dict, dfd) -> dict:
    ...
    results = fi.search_keywords("@EnableZuulServer", file_extension=["*.java", "*.kt", "*.scala"])
    new_results = fi.search_keywords("@EnableZuulProxy", file_extension=["*.java", "*.kt", "*.scala"])
    ...
    for r in results.keys():
        ...
        for m in microservices.values():
            if m["name"] == zuul_server:    # this is the Zuul server
                # FIXME: insertion un peu étrange -> zuul n'implique pas forcémenet de loadbalancer
                m["stereotype_instances"] = m.get("stereotype_instances", []) + ["gateway"]
                # , "load_balancer"]
                m["tagged_values"] = m.get("tagged_values", []) + [("Gateway", "Zuul")]

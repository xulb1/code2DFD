...
def get_incoming_endpoints(dfd) -> set:
    ...
    files = fi.search_keywords(["@KafkaListener", "@StreamListener"], file_extension=["*.java"])
...
def get_outgoing_endpoints(dfd) -> set:
    ...
    for template in kafkatemplates:
        files = fi.search_keywords(f"{template}.send", file_extension=["*.java"])
        for fpath, file in files.items():
            # print(fpath)
            microservice = tech_sw.detect_microservice(file["path"], dfd)
    ...
    files = fi.search_keywords("@SendTo", file_extension=["*.java"])
    for fpath, file in files.items():
        microservice = tech_sw.detect_microservice(file["path"], dfd)

...
def used_in_application():
    return len(fi.search_keywords("RestTemplate", file_extension=["*.java", "*.kt", "*.scala"])) > 0
...
def get_incoming_endpoints(dfd) -> list:
    ...
    endpoints = set()
    files = fi.search_keywords("RequestMapping", file_extension=["*.java", "*.kt", "*.scala"])
...
def get_outgoing_endpoints(information_flows: dict, dfd) -> set:
    ...
    for command in commands:
        files = fi.search_keywords(command, file_extension=["*.java", "*.kt", "*.scala"])
        for file in files.keys():
            f = files[file]
...
def find_rst_variable(parameter: str, file: dict, line_nr: int, information_flows: dict, microservice: str, dfd, count=0):
    """
    Resolves the value of a RestTemplate endpoint parameter, handling variables, class references, and input parameters.
    
    Attempts to statically determine the endpoint URL by recursively searching for variable assignments and class attributes. 
    Updates the information_flows dictionary if a service name is detected in the parameter.
    """
    ...
    for p in range(len(parameters)):    # Treat each part individually
        ...
        if "(" in parameters[p] or ")" in parameters[p]:
            ...
            try:
                ...
                files_containing_class = fi.search_keywords(f"class {parameter_class}", file_extension=["*.java", "*.kt", "*.scala"])
                correct_file = None

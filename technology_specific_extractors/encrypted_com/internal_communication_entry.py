...
def check_inter_service_encryption(microservices: dict, information_flows: dict) -> dict:
    ...
    for m in microservices.values():
        ...
        for mot in ENCRYPTION_KEYWORDS["server"]["config"]:
            ...
            if results:
                for i in results["content"]:
                    print(i)
        is_server_ssl_enabled = any(fi.search_keywords(keyword, directory_path, file_extension=["*.java", "*.kt", "*.scala"]) for keyword in ENCRYPTION_KEYWORDS["server"]["java_code"]) or \
        ...
        is_client_ssl_enabled = any(fi.search_keywords(keyword, directory_path) for keyword in ENCRYPTION_KEYWORDS["client"]["all"]) or \
                                ...
                                any(fi.search_keywords(keyword, directory_path,file_extension=["*.java", "*.kt", "*.scala"]) for keyword in ENCRYPTION_KEYWORDS["client"]["java_code"])

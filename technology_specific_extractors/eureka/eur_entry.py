...
def detect_eureka(microservices: dict, information_flows: dict, dfd) -> dict:
    ...
    for keyword in SERVER_KEYWORDS:
        # print("\n\n================================================================================================")
        results = fi.search_keywords(keyword, file_extension=["*.java", "*.kt", "*.scala","*.conf","*.sh","*.xml","*.gradle","*.json","*.yml","*.yaml","*.properties"])
...
def detect_eureka_v0(microservices: dict, information_flows: dict, dfd) -> dict:
    ...
    server_results = fi.search_keywords("@EnableEurekaServer", file_extension=["*.java", "*.kt", "*.scala"])
    eureka_servers = []
    ...
    result_paths = set()
    results = fi.search_keywords(["@EnableEurekaClient","@EnableDiscoveryClient"], file_extension=["*.java", "*.kt", "*.scala"])
    for res in results.values():
...
def is_eureka(microservice: tuple) -> bool:
    ...
    files = fi.search_keywords("@EnableEurekaServer", file_extension=["*.java", "*.kt", "*.scala"])
...
def detect_eureka_server_only(microservices: dict, dfd):
    ...
    results = fi.search_keywords("@EnableEurekaServer", file_extension=["*.java", "*.kt", "*.scala"])
    eureka_servers = set()
